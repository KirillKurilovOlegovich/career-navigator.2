...
@section('content')
    <main class="flex flex-col gap-14 py-10 container mx-auto max-w-[1280px] px-4">
        ...
        <form action="{{ route('update_quiz', $quiz->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="flex flex-col gap-2 w-2/4">  <!-- Используем flex-col для вертикального расположения -->
        <label for="testTitle" class="form-label">Название теста:</label>
        <input type="text" class="form-control" value="{{ $quiz->title }}" id="testTitle" name="title" required>
        ...
        <button type="submit" class="bg-red-800 text-white rounded-md w-full">Сохранить изменения</button>
    </div>
</form>
